...
_SIGNALS: list[tuple[float, re.Pattern[str]]] = [
    ...
    (
        0.95,
        re.compile(
            r"disregard\s+(all\s+|any\s+)?(previous|prior|above|earlier|your)\s+((previous|prior|above|earlier)\s+)?(instructions?|prompts?|rules?|guidelines?)",
            re.I,
        ),
    ),
    (
        0.90,
        re.compile(
            r"(reveal|show|print|repeat|output|leak)\b.{0,40}\b(system\s+prompt|hidden\s+instructions?|initial\s+prompt)",
            re.I,
        ),
    ),
    (
        0.90,
        re.compile(
            r"\byou\s+are\s+now\b.{0,60}\b(DAN|jailbroken|unrestricted|without\s+(any\s+)?(rules|restrictions|filters))",
            re.I,
        ),
    ),
    ...
    (
        0.80,
        re.compile(
            r"\b(developer|debug|god|admin)\s+mode\b.{0,40}\b(enabled?|activated?|on)\b", re.I
        ),
    ),
    (
        0.80,
        re.compile(
            r"\bpretend\s+(that\s+)?you\s+(are|have)\b.{0,60}\b(no\s+(rules|restrictions|guidelines)|evil|unfiltered)",
            re.I,
        ),
    ),
    ...
    (
        0.50,
        re.compile(
            r"\brespond\s+only\s+with\b.{0,40}\b(json|code|yes)\b.{0,40}\bno\s+matter\s+what\b",
            re.I,
        ),
    ),
]
...
def is_blocked(injection_score: float, threshold: float, mode: str) -> bool:
    ...
    ``block`` blocks at or above the threshold; any unrecognized mode
    fails closed (blocks unconditionally), so a typo in the configured
    mode can never silently disable the guardrail.
    """
    if mode == "flag_only":
        return False
    if mode == "block":
